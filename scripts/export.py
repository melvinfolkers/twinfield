...
from scripts.run_settings import get_blob_settings
...
def push_to_azure(df, tablename):
    ...
    result = 'push successful ({}):'.format(tablename), len(
        df), 'records pushed to Microsoft Azure', '({} columns)'.format(numberofcolumns)
    logging.info(result)
